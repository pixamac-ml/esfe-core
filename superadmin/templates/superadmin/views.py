...
# ============================================
# VIEWS MANQUANTES - AJOUTÉES POUR PRODUCTION
# ============================================
...
def fee_delete(request, pk):
    ...
    if request.method == 'POST':
        fee = get_object_or_404(Fee, pk=pk)
        fee.delete()
        messages.success(request, "Frais supprimés!")
...
def category_create(request):
    ...
    if request.method == 'POST':
        ...
        if name:
            BlogCategory.objects.create(name=name, description=description, slug=slugify(name))
            messages.success(request, f"Catégorie '{name}' créée!")
            return redirect('superadmin:category_list')
    
    context = {'page_title': 'Nouvelle Catégorie'}
    return render(request, 'superadmin/categories/form.html', context)
...
def branch_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        name = branch.name
        branch.delete()
        messages.success(request, f"Campus '{name}' supprimé!")
...
def settings(request):
    ...
    if request.method == 'POST':
        if institution:
            ...
            messages.success(request, "Institution créée!")
        return redirect('superadmin:settings')
...
def search_global(request):
    ...
    if query and len(query) >= 2:
        # Rechercher dans les modèles principaux
        results['formations'] = Programme.objects.filter(
            Q(title__icontains=query) | Q(description__icontains=query)
        )[:5]
        ...
        results['candidatures'] = Candidature.objects.filter(
            Q(user__first_name__icontains=query) |
            Q(user__last_name__icontains=query)
        )[:5]
    ...
    return render(request, 'superadmin/search_results.html', context)
...
def bulk_action(request):
    ...
    if not selected_ids or not action:
        messages.error(request, "Aucune action sélectionnée.")
        return redirect('superadmin:dashboard')
    ...
    try:
        if model_type == 'formation':
            if action == 'delete':
                count = Programme.objects.filter(id__in=selected_ids).delete()[0]
            elif action == 'activate':
                count = Programme.objects.filter(id__in=selected_ids).update(is_active=True)
            elif action == 'deactivate':
                count = Programme.objects.filter(id__in=selected_ids).update(is_active=False)
        
        elif model_type == 'candidature':
            if action == 'delete':
                count = Candidature.objects.filter(id__in=selected_ids).delete()[0]
        
        elif model_type == 'article':
            if action == 'delete':
                count = Article.objects.filter(id__in=selected_ids).delete()[0]
            elif action == 'publish':
                count = Article.objects.filter(id__in=selected_ids).update(status='published')
...
def export_data(request, model_type):
    ...
    import csv
    from django.http import HttpResponse as CsvHttpResponse
    
    response = CsvHttpResponse(content_type='text/csv')
    
    if model_type == 'students':
        response['Content-Disposition'] = 'attachment; filename="etudiants.csv"'
        writer = csv.writer(response)
        writer.writerow(['Matricule', 'Nom', 'Email', 'Date création'])
        ...
        for student in Student.objects.select_related('user'):
            writer.writerow([
                ...
                student.user.email,
                student.created_at.strftime('%d/%m/%Y') if hasattr(student, 'created_at') else '',
            ])
        ...
        for cand in Candidature.objects.select_related('user', 'programme'):
            writer.writerow([
                cand.id,
                cand.user.get_full_name() if cand.user else '',
                cand.programme.title if cand.programme else '',
                cand.status,
                cand.created_at.strftime('%d/%m/%Y'),
            ])
    
    elif model_type == 'payments':
        response['Content-Disposition'] = 'attachment; filename="paiements.csv"'
        writer = csv.writer(response)
        writer.writerow(['Référence', 'Étudiant', 'Montant', 'Statut', 'Date'])
        
        for payment in Payment.objects.select_related('student__user'):
            writer.writerow([
                payment.reference,
                payment.student.user.get_full_name() if payment.student else '',
                payment.amount,
                payment.status,
        ...
        return CsvHttpResponse("Type d'export non supporté", status=400)
